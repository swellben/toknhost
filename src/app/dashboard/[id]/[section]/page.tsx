...
import { VALID_DESIGN_SYSTEM_SECTIONS, type DesignSystemSection } from "@/lib/design-system-sections";
...
export default async function DesignSystemSectionPage({
  ...
  if (!VALID_DESIGN_SYSTEM_SECTIONS.has(section)) notFound();
  const activeSection = section as DesignSystemSection;
  ...
  // all at once.
  const [{ data: values }, { data: allValues }, { data: a11yChecks }, previewTheme, previewThemeShadcn, previewThemeTailwindV4] =
    await Promise.all([
      ...
      tokens?.length
        ...
        : Promise.resolve(null),
      tokens?.length
        ? fetchPreviewTheme(ds.slug, ds.mcp_token, ds.is_public, "tailwind-v4")
        : Promise.resolve(null),
    ]);
  ...
  // Hidden from the Primitives page display only — still stored, gap-filled,
  // and exported normally. Same categories hidden for every design system
  // regardless of target_framework — not enough V1 design-system targets
  // surface these categories yet to make showing them here useful.
  const HIDDEN_PRIMITIVE_CATEGORIES = new Set(["animation", "drop-shadow", "duration", "easing", "transition"]);

  for (const t of tokens ?? []) {
    const bucket = classifyToken(t.category, t.path);
    if (bucket === "typography") typographyTokens.push(t);
    else if (bucket === "primitives") {
      if (!HIDDEN_PRIMITIVE_CATEGORIES.has(t.category)) primitiveTokens.push(t);
    }
  ...
  const modeSwitcher = modes && modes.length > 1 && (
    <div className="flex gap-1 rounded-md bg-muted p-1">
      {modes.map((m) => (
        <Link
          key={m.id}
          href={`/dashboard/${id}/${activeSection}?mode=${m.name}`}
  ...
  // Order: the two singular brand-color rows first (color.primary,
  // color.secondary exactly), then their .50-.950 ramps (primary ramp before
  // secondary ramp), then the two foreground rows, then everything else,
  // with chart colors pushed to the very bottom.
  function colorRootPriority(path: string): number {
    if (path === "color.primary") return 0;
    if (path === "color.secondary") return 1;
    if (path === "color.primary.foreground") return 4;
    if (path === "color.secondary.foreground") return 5;
    const root = path.split(".")[1] ?? "";
    if (root === "primary") return 2;
    if (root === "secondary") return 3;
    if (root === "chart") return 7;
    return 6;
  }

  function sortColorsByPath(a: { path: string }, b: { path: string }): number {
    const aPriority = colorRootPriority(a.path);
    const bPriority = colorRootPriority(b.path);
    const priorityDiff = aPriority - bPriority;
    if (priorityDiff !== 0) return priorityDiff;
    // Within a primary/secondary ramp (priority 2/3), darkest step first —
    // reverse of the usual ascending 50→950 (lightest→darkest) order.
    if (aPriority === 2 || aPriority === 3) return sortByPath(b, a);
    ...
    if (!list?.length) {
      return (
        <p className="text-sm text-muted-foreground">
          No color tokens yet — import tokens or run gap-fill from the Details section.
        </p>
      );
    }
        ...
        {[...groupByCategory(list).entries()].map(([category, categoryTokens]) => (
          <div key={category} className="flex flex-col gap-2">
            ...
            <table className="w-full text-sm table-fixed">
              ...
              <tbody>
                {[...categoryTokens!].sort(sortColorsByPath).map((t) => {
    ...
    case "primitives":
      content = (
        <Card>
          ...
          <CardContent>
            {tokenTable(primitiveTokens, "No primitives yet — import tokens or run gap-fill from the Details section.")}
          </CardContent>
        </Card>
      );
      break;
    case "tokens":
      content = (
        <Card>
          ...
          <CardContent>
            {tokenTable(semanticTokens, "No semantic tokens yet — import tokens or run gap-fill from the Details section.")}
          </CardContent>
        </Card>
      );
      break;
    ...
    case "copy":
      content = (
        <Card>
          ...
          <CardContent>
            <p className="text-sm text-muted-foreground">
              This section isn&apos;t built yet — see PIVOT-PLAN.md for status.
            </p>
          </CardContent>
        </Card>
      );
        ...
        <div className="flex flex-1 gap-6 overflow-hidden">
          ...
          <div className="w-1/2 shrink-0 overflow-y-auto border-l pl-6">
            <DesignSystemPreviewPanel
              ...
              tailwindV4ByMode={previewThemeTailwindV4}
            />
          </div>
        </div>
      </ThemeProvider>
