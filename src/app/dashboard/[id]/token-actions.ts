...
export async function updateTokenValue(
  ...
  // Deliberately does NOT regenerate this color's scale here — that used to
  // happen on every edit, which would mean every AI color-fill run costs an
  // AI call per save. Per the decided design (PIVOT-PLAN.md "a new 'Update
  // design system' flow replaces ad-hoc per-token AI regeneration"), editing
  // a swatch just saves the raw value; the full regeneration only fires on
  // the explicit "Update design system" action (see `updateDesignSystem` in
